...
export function Navbar({
  ...
  const containerStyle: React.CSSProperties = {
    ...
    boxShadow:
      variant === "solid" && sticky
        ? "0 1px 0 rgba(15, 23, 42, 0.04), 0 8px 24px -16px rgba(15, 23, 42, 0.12)"
        : undefined,
  };

  // Layout: where the links sit relative to the brand.
        ...
        >
          {brandIcon && (
            <span
              ...
              style={{
                background:
                  variant === "transparent" ? "rgba(15,23,42,0.06)" : "rgba(15,23,42,0.06)",
                color: textColor,
              }}
            >
              {brandIcon}
            ...
            {items.map((item, idx) => (
              <li key={`${item.label}-${idx}`}>
                <a
                  href={item.href}
                  onClick={(e) => e.preventDefault()}
                  className="inline-flex items-center gap-1.5 rounded-md px-3 py-2 text-sm font-medium opacity-80 transition-opacity hover:opacity-100"
      ...
      {items.length > 0 && mobileOpen && (
        <div
          ...
          style={{
            borderColor,
            background:
              variant === "transparent" ? "rgba(255,255,255,0.96)" : background,
          }}
        >
    ...
    <div className="flex flex-col gap-4">
      ...
      <FieldRow label="Brand mark (single emoji or letter)">
        <Input
          ...
          onChange={(e) =>
            setProp((props: NavbarProps) => {
              props.brandIcon = e.target.value;
            })
          }
          placeholder="🚀"
        />
              ...
              >
                <div className="grid grid-cols-[1fr_1fr_auto] gap-1.5">
                  ...
                  <Input
                    ...
                    placeholder="/path"
                    onChange={(e) =>
                      updateItem(idx, { href: e.target.value })
                    }
                  />
                  <button
                    type="button"
      ...
      <SelectField
        ...
        onChange={(v) =>
          setProp((props: NavbarProps) => {
            props.layout = v as NavbarProps["layout"];
          })
        }
        options={[
          {
            value: "brand-left-links-right",
            label: "Brand left, links right",
          },
          {
            value: "brand-left-links-center",
            label: "Brand left, links centered",
          },
      ...
      <SelectField
        ...
        onChange={(v) =>
          setProp((props: NavbarProps) => {
            props.variant = v as NavbarProps["variant"];
          })
        }
        options={[
